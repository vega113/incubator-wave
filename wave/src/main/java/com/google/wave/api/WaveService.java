...
public class WaveService {
  ...
  /** A map of RPC server URL to its JWT bearer token. */
  private final Map<String, String> jwtTokenMap = new HashMap<String, String>();
  ...
  public void setupOAuth(String consumerKey, String consumerSecret, String rpcServerUrl) {
    ...
    jwtTokenMap.remove(rpcServerUrl);
    consumerDataMap.put(rpcServerUrl, new ConsumerData(consumerKey, consumerSecret, rpcServerUrl));
  }

  /**
   * Sets the JWT bearer token used for outgoing operations to the given RPC server URL.
   *
   * <p>This is the JWT equivalent of {@link #setupOAuth(String, String, String)}. The
   * configured token applies only to the exact RPC URL that is passed here, which allows callers
   * to use different tokens for the Data API and the Active Robot API.
   *
  ...
  public void setupJwt(String token, String rpcServerUrl) {
    if (token == null || rpcServerUrl == null) {
      throw new IllegalArgumentException("Token and RPCServerURL have to be non-null");
    }
    if (token.trim().isEmpty()) {
      throw new IllegalArgumentException("Token has to be non-empty");
    }
    consumerDataMap.remove(rpcServerUrl);
    jwtTokenMap.put(rpcServerUrl, token);
  }
  ...
  private List<JsonRpcResponse> makeRpc(OperationQueue opQueue, String rpcServerUrl)
    ...
    String jwtToken = jwtTokenMap.get(rpcServerUrl);
    ...
    try {
      ...
      if (jwtToken != null) {
        ...
        HttpMessage request = new HttpMessage("POST", new URL(rpcServerUrl), bodyStream);
        request.headers.add(
            new SimpleEntry<String, String>(HttpMessage.CONTENT_TYPE, JSON_MIME_TYPE));
        request.headers.add(
            new SimpleEntry<String, String>("Authorization", "Bearer " + jwtToken));
        responseStream =
            httpFetcher.execute(request, Collections.<String, Object>emptyMap()).getBody();
      } else if (!consumerDataObj.isUserAuthenticated()) {

 ...
 * in a {@link com.google.wave.api.event.DocumentChangedEvent}, and would like to
 * get the parent blip with the incoming event bundle, then it should implement
 * this method:
 * <pre>
 *   {@literal @}Capability(contexts = {Context.PARENT, Context.SELF})
 *   public void onDocumentChanged(DocumentChangedEvent e) {
 *     // Skip intermediate typing deltas — only respond when editing is complete.
 *     boolean isEditingComplete = e.getBlip().getAnnotations().asList().stream()
 *         .noneMatch(a -> a.getName() != null &amp;&amp; a.getName().startsWith("user/d/"));
...
public abstract class AbstractRobot extends HttpServlet implements EventHandler {
    ...
    for (Event event : events.getEvents()) {
      switch (event.getType()) {
        ...
        case BLIP_CONTRIBUTORS_CHANGED:
          ...
          break;
        case BLIP_SUBMITTED:
          // Deprecated: BLIP_SUBMITTED events are never generated by modern Wave.
          // This branch is kept for binary compatibility only and will never execute.
          // Use DOCUMENT_CHANGED with a user/d/ annotation check instead.
  ...
  public void onBlipSubmitted(BlipSubmittedEvent event) {
    // No-op. BLIP_SUBMITTED is never generated by modern Wave — this method is kept
    // for binary compatibility only. Override onDocumentChanged() instead and check
    // for the absence of a "user/d/" annotation to detect editing completion.
  }

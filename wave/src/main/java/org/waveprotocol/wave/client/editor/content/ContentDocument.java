...
public class ContentDocument {
  ...
  public class PersistentContentDoc extends PersistentContentView {
    ...
    void setPersistenceManager(LazyPersistenceManager persistenceManager) {
      Preconditions.checkArgument(persistenceManager != null, "persistenceManager != null")
          "Can't use a null persistence manager.");
      Preconditions.checkState(this.persistenceManager == null, "this.persistenceManager == null")
...
//    };
  ...
  public void setOutgoingSink(SilentOperationSink<? super DocOp> outgoingOperationSink) {
    ...
    Preconditions.checkState(this.outgoingOperationSink == SilentOperationSink.Void.get(), "this.outgoingOperationSink == SilentOperationSink.Void.get()")
        "Already has a sink");
    ...
    for (int i = level.ordinal() + 1; i <= newLevel.ordinal(); i++) {
      ...
      switch (currentLevel) {
        ...
        case INTERACTIVE:
          ...
          Preconditions.checkState(fullRawSubstrate.getAffectHtml(), "fullRawSubstrate.getAffectHtml()")
  ...
  public void attachEditor(MiniBundle editorBundle, LogicalPanel panel) {
    ...
    Preconditions.checkState(level != Level.EDITING, "level != Level.EDITING")
        "Cannot attach editor to a document already with an editor");

    if (panel == null) {
      Preconditions.checkState(this.logicalPanel != null, "this.logicalPanel != null")

...
public final class ThreadNavigationPresenter {
  ...
  private void onHistoryTokenChanged(String token) {
    ...
    String depthStr = extractHistoryParam(token, HISTORY_PARAM_DEPTH);
    if (depthStr != null) {
      try {
  ...
  public void reconcileFocusedThreadLayout(TopConversationView waveUi) {
    ...
    String focusedBlipId = extractHistoryParam(History.getToken(), HISTORY_PARAM_FOCUS);
  ...
  private String extractHistoryParam(String token, String key) {
    ...
    for (String part : parts) {
      ...
      if (key.equals(part.substring(0, idx))) {
        return URL.decodeQueryString(part.substring(idx + 1));
      }
    }
    return null;
  }

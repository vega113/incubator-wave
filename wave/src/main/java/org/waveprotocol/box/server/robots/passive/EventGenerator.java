 ...
 * <li>BlipEditingDone (DONE — synthesized when all user/d/ annotations have end timestamps)</li>
 *
...
public class EventGenerator {
  ...
  private class EventGeneratingDocumentHandler implements DocHandler {
    ...
    /**
     * Set to true if a {@link BlipEditingDoneEvent} has been generated by this
     * handler (at most one per bundle).
     */
    private boolean blipEditingDoneEventGenerated;
    ...
    public void onDocumentEvents(EventBundle<N, E, T> event) {
      ...
      for (DocumentEvent<N, E, T> eventComponent : eventComponents) {
        if (eventComponent.getType() == DocumentEvent.Type.ANNOTATION_CHANGED) {
          ...
          // Synthesize BLIP_EDITING_DONE when the last active user/d/ session closes.
          if (!blipEditingDoneEventGenerated
              && capabilities.containsKey(EventType.BLIP_EDITING_DONE)
              && anotationChangedEvent.key.startsWith("user/d/")
              && anotationChangedEvent.newValue != null) {
            String[] parts = anotationChangedEvent.newValue.split(",", 3);
            if (parts.length == 3 && !parts[2].isEmpty()) {
              // End timestamp was just filled in — check if ALL user/d/ sessions are closed.
              if (allEditingSessionsClosed(doc)) {
                BlipEditingDoneEvent editDoneEvent = new BlipEditingDoneEvent(
                    null, null, deltaAuthor.getAddress(), deltaTimestamp, blip.getId());
                addEvent(editDoneEvent, capabilities, blip.getId(), messages);
                blipEditingDoneEventGenerated = true;
              }
            }
          }
        } else {
    ...
    private boolean allEditingSessionsClosed(ObservableDocument doc) {
      ...
      doc.knownKeys().each(key -> {
        ...
        if (key.startsWith("user/d/")) {
          int pos = doc.firstAnnotationChange(0, doc.size(), key, null);
          if (pos >= 0) {

...
public final class SearchIndexerTest extends TestCase {
  ...
  public void testKnownWaveStillTriggersOtherParticipantSubscriptions() {
    ...
    SearchIndexer.SubscriptionKey activeKey =
        new SearchIndexer.SubscriptionKey(user, SearchWaveletManager.md5Hex("in:inbox"));
    SearchIndexer.SubscriptionKey unrelatedKey =
        new SearchIndexer.SubscriptionKey(user, SearchWaveletManager.md5Hex("is:unread"));

    indexer.registerSubscription(
        user,
        "in:inbox",
        activeKey.getQueryHash(),
        ImmutableSet.of(indexedWave));
    indexer.registerSubscription(
        user,
        "is:unread",
        unrelatedKey.getQueryHash(),

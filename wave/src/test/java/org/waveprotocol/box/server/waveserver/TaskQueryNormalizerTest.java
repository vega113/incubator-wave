...
public final class TaskQueryNormalizerTest extends TestCase {
  ...
  public void testNormalizeAppendsLocalDomainForBareNames() {
    assertEquals(
        "alice@example.com",
        TaskQueryNormalizer.normalize("Alice", ParticipantId.ofUnsafe("user@example.com")));
  }

  public void testNormalizeLowercasesExplicitAddresses() {
    assertEquals(
        "alice@example.com",
        TaskQueryNormalizer.normalize("Alice@Example.com",
            ParticipantId.ofUnsafe("user@example.com")));
  }

  public void testNormalizeRejectsEmptyInput() {
    try {
      TaskQueryNormalizer.normalize("", ParticipantId.ofUnsafe("user@example.com"));
      fail("Expected empty task value to be rejected");
    } catch (IllegalArgumentException expected) {
      assertTrue(expected.getMessage().contains("raw task value"));
    }
  }

  public void testNormalizeRejectsNullInput() {
    try {
      TaskQueryNormalizer.normalize(null, ParticipantId.ofUnsafe("user@example.com"));
      fail("Expected null task value to be rejected");
    } catch (IllegalArgumentException expected) {
      assertTrue(expected.getMessage().contains("raw task value"));
    }
  }
  ...
  public void testNormalizeRejectsNullUser() {
    try {
      TaskQueryNormalizer.normalize("me", null);

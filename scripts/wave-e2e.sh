...
# Copy JUnit XML reports (generated by sbt-jupiter-interface) to results dir
if compgen -G "wave/target/e2eTest-reports/*.xml" > /dev/null 2>&1; then
  cp wave/target/e2eTest-reports/*.xml "$RESULTS_DIR/"
  echo "[e2e] JUnit XML reports copied to $RESULTS_DIR/"
elif compgen -G "wave/target/test-reports/*.xml" > /dev/null 2>&1; then
  cp wave/target/test-reports/*.xml "$RESULTS_DIR/"
  echo "[e2e] JUnit XML reports copied to $RESULTS_DIR/"
fi
